...
#let portada(meta) = page(margin: (x: 2.4cm, top: 3cm, bottom: 2.4cm))[
  #align(center + horizon)[
    #text(weight: "bold", size: 10pt, tracking: 1pt)[#upper(meta.tipo-largo)]
    #v(1cm)
    #text(weight: "bold", size: 26pt)[#meta.titulo]
    #v(0.3cm)
    #text(size: 14pt, fill: gris-texto)[#meta.subtitulo]
    #v(1.5cm)
    #text(size: 10pt)[#codigo-base(meta) · v#meta.version · #meta.fecha]
  ]
]
...
#let report(meta: (:), doc) = {
  ...
  set page(
    ...
    footer: context {
      ...
      grid(
        columns: (1fr, 1fr, 1fr),
        align(left)[#text(size: 8pt)[#codigo-base(meta) · v#meta.version]],
        [],
        align(right)[#text(size: 8pt)[#counter(page).display()]],
      )
    },
  )
  portada(meta)
